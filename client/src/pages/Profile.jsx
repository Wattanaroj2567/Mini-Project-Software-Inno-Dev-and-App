...
import AutoWidthTextField, {
  measureCh,
} from "@/components/common/AutoWidthTextField.jsx";
...
export default function Profile() {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          const normalized = {
            ...data.data,
            profileImage: resolveProfileImagePath(data.data.profileImage),
          };
  ...
  async function saveDisplayName() {
    ...
    try {
      ...
      const updated = {
        ...profile,
        ...data.data,
        profileImage: resolveProfileImagePath(
          data.data?.profileImage || profile.profileImage
        ),
      };
      setProfile(updated);
  ...
  const ProfileTabs = () => (
    ...
    >
      ...
      <Tab
        label="ชื่อผู้ใช้"
        sx={{ display: profile?.isGoogleUser ? "none" : undefined }}
      />
      ...
      <Tab
        ...
        sx={{
          display:
            profile?.isGoogleUser || !profile?.hasLocalPassword
              ? "none"
              : undefined,
        }}
      />
      ...
      <Stack spacing={3}>
        ...
        {isMobile ? (
          <>
            {/* Mobile Layout */}
            <ProfileTabs />
            <Box sx={{ mt: 2 }}>{renderProfileContent()}</Box>
          </>
        ) : (
          /* Desktop Layout */
      ...
      <>
        ...
        {active === 2 && (
          <Stack spacing={2}>
            {profile?.isGoogleUser ? (
              <>
                <Alert severity="info">
                  บัญชีนี้เชื่อมต่อกับ Google แล้ว ({profile.email})
                </Alert>
              ...
              <>
                {emailMsg && (
                  <Alert
                    severity={
                      emailMsg.startsWith("สำเร็จ") ? "success" : "error"
                    }
                  >
                    {emailMsg}
                  </Alert>
                ...
                {(() => {
                  const widthCh = measureCh(profile.email || "", {
                    min: 16,
                    max: 60,
                    pad: 2,
                  });
                  return (
                    <>
                      ...
                      <TextField
                        ...
                        sx={{ width: `${widthCh}ch`, maxWidth: "100%" }}
                      />
                      <Button
                        variant="contained"
                        size="large"
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            setEmailMsg(
                              "สำเร็จ: " +
                                (data?.message || "เปลี่ยนอีเมลสำเร็จ")
                            );
                            setProfile((p) => ({
                              ...p,
        ...
        {active === 3 && (
          <Stack spacing={2} maxWidth={520}>
            {pwdMsg && (
              <Alert
                severity={pwdMsg.startsWith("สำเร็จ") ? "success" : "error"}
              >
                {pwdMsg}
              </Alert>
            )}
          ...
          <Stack spacing={2} maxWidth={520}>
            <Grid container spacing={2} alignItems="center">
              <Grid item>
                <Avatar
                  src={toApiAsset(
                    resolveProfileImagePath(profile.profileImage)
                  )}
            ...
            {file && (
              <Button
                ...
                onClick={async () => {
                  await saveAvatar();
                  notifySuccess("บันทึกรูปโปรไฟล์ใหม่แล้ว");
                }}
                fullWidth
              >
                บันทึกรูปใหม่
  ...
  return (
    <Stack spacing={2} maxWidth={520}>
      ...
      <Typography color="text.secondary">
        คำเตือน: การลบบัญชีจะเป็นการลบข้อมูลผู้ใช้และรีวิวทั้งหมดอย่างถาวร
        ไม่สามารถกู้คืนได้
      </Typography>
      <Button
        variant="contained"
      ...
      <Dialog open={open} onClose={() => setOpen(false)}>
        ...
        <DialogContent>
          <DialogContentText>
            การดำเนินการนี้ไม่สามารถย้อนกลับได้ หากต้องการยืนยัน ให้พิมพ์คำว่า
            DELETE แล้วกดปุ่มยืนยัน
          </DialogContentText>
          <TextField
            autoFocus
            margin="dense"
